...
struct vertex_iterator_t {
    ...
    typedef vertex_iterator_t <T> self;
    typedef T reference;
    typedef T * pointer;

    friend class graph_t<T>;
private:
    vertex_iterator_t(table_iterator x)
    {
        _iter = x;
    }
...
public:
    ...
    vertex_iterator_t(const self& x)
    {
        _iter = x._iter;
    }
    reference operator *() const
 ...
 */
template <typename T>
struct vertex_const_iterator_t {
    typedef typename bsl::hashmap<T, adjacency_node<T> *>::const_iterator table_const_iterator;
    typedef vertex_const_iterator_t <T> self;
    typedef T reference;
    typedef T * pointer;

    friend class graph_t<T>;
private:
    vertex_const_iterator_t(table_const_iterator x)
    {
        _iter = x;
    }
...
public:
    ...
    vertex_const_iterator_t(const self& x)
    {
        _iter = x._iter;
    }
    reference operator *() const
...
public:
    ...
    typedef vertex_iterator_t<T> vertex_iterator;
    typedef vertex_const_iterator_t<T> vertex_const_iterator;
