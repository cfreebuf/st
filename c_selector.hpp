...
template <class _TS1, class _TS2> struct ts_less_than {
    static const bool R =
        (CAP(getSeekScore, typename _TS1::SeekInfo) >
         CAP(getSeekScore, typename _TS2::SeekInfo));
};
...
template <class _From, class _Where> class Selector2 : public c_show_base {
    typedef typename _From::ArgL _MTableL;
    typedef typename _Where::PredL _PredL;
