...
class TableIndexEraser {
    typedef typename _IndexTup::template type_at_n<_Seq>::R Index;
    ...
    const char* filename, char sep, bool abort_if_any_error, size_t max_line)
{
    ...
    for (ssize_t n_read = 0;
         ...
         ++line_no) {
        if (line_no > max_line) {
            ST_WARN("exceed the max line limit %zu. abort", max_line);
            break;
        }
        ...
        if (buf[n_read] == '\0' &&  // ended with zero
            ...
            rc = -1;
            ST_FATAL("Fail to parse line %zu: \"%s\", abort", line_no, buf);
            break;
        } else {
            rc = 1;
            ST_WARN("Fail to parse line %zu: \"%s\"", line_no, buf);
        }
    }
