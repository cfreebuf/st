...
class ConnectorStack
{
    ...
    struct insert
    {
        ...
        {                                                              
            ...
            // FIXME(lidm): Table
            typedef TCAP(content_type_of, typename Tup::template type_at_n<N - 2>::R) Table2;
            insert<Table2, Tup, N - 1>::call(that, tup, sid);
        }
    };
    ...
    struct insert<TableStack<T>, Tup, N>
    {
        static void call(ConnectorStack<CONNECTOR> & that, Tup & tup, int sid)
        {
            ...
            typedef typename TableStack<T>::iterator iterator;
