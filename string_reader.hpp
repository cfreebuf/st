...
namespace aux {
struct ZeroSized {
    // FIXME(lidm):compile error occures here!
    char dummy[0];
};
}
...
template <typename T> Many<T> many0(T v, int max_times = INT32_MAX)
{ return Many<T>(v, 0, max_times); }

template <typename T> Many<T> many1(T v, int max_times = INT32_MAX)
