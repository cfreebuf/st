...
struct init_join_iterator<Cons<_TS, _TSL>, _JID> {
    static bool call (typename _JID::IterTup* p_iter_tup,
                      const typename _JID::VarTup& var_tup,
                      const typename _JID::TablePtrTup& table_tup)
    {
        ...
        do {
            CAP(list_at, _TS::TABLE_POS, class _JID::IterMakerL)::make(
                ...
                *p_iter_tup, var_tup);
            for ( ; it && !check_filter_list<typename _TS::FPredL,
                                             typename _JID::IterTup,
                                             typename _JID::VarTup>
...
{      
    ...
    if (check_filter_list<typename _JID::PreFPredL, IterTup, VarTup>
...
const typename _Attr::Type& JoinIterator<_TSL, _JID>::at () const {
    return iter_tup_.template at_n<_TBLn<_Attr>::TABLE_POS>()
        ->template at<_Attr>();  // FIXME(lidongming):add template
}

template <class _TSL, class _JID>
template <class _TableRep>
const typename _TableRep::Attr::Type& JoinIterator<_TSL, _JID>::at () const
{ return iter_tup_.template at_n<_TableRep::TABLE_POS>()
    // FIXME(lidongming):add template
    ->template at<typename _TableRep::Attr>(); }
